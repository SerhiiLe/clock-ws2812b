...
// Назначение PIO для разных типов микроконтроллеров. Сверяйтесь с pinout вашего модуля.
	...
	#define PIN_RELAY 26 // реле выключатель питания матрицы
		...
		#define STX 17 // hardware serial RX DFPlayer. Закомментировать, если не подключен
...
#define AP_SSID "ClockAP" // имя точки доступа, к которой подключается телефон для начальной настройки
// #define AP_PASSWORD "12345678" // пароль этой точки доступа, если не задан, то точка доступа без пароля

/*** часовой пояс и летнее время. Можно переопределить через Web настройки ***/

#define TIMEZONE 2 // часовой пояс по умолчанию
...
#define NET_RETRY_COUNT 5 // количество повторов при сбое сети (нельза получить цитаты, погоду)
#define NET_RETRY_TIME 60 // время для повтора после сбоя в секундах
#define NET_RETRY_PENALTY 3600 // время приостановки повторных запросов после NET_RETRY_COUNT попыток. В секундах
