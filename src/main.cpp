...
TimerMinim telegramTimer(1000U * ts.tb_rate);	// период опроса команд из Телеграм
...
TimerMinim timerDclickReset(30 * 1000UL);	// сброс действия двойного клика на начало цикла
TimerMinim timerCheckMp3Play(200);			// проверка, точного состояния dfplayer
...
void alarms_pool() {
	...
	if(alarmTimer.isReady()) {
		...
		if (fl_run_allow && t.tm_min == 0 && cs.enable && cuckoo_last != t.tm_hour) {
			...
			if (timerCheckMp3Play.isReady()) mp3_isPlay(); // нужно для чипов с проблемной обратной связью
		...
		case 2:
			...
			if(fl_5v) { // при низком напряжении сенсорная кнопка может давать ложные срабатывания, по этому отключить все функции
				if(fl_password_reset_req) {
					...
					if (timerDclickReset.isReady()) dclick_func = 0;
					uint8_t current = 0;
					bool execute = false;
					...
					if (!execute)
					for(uint8_t i=0; i<MAX_TMP_MESSAGES; i++) {
					...
					if (!execute)
						fl_demo = !fl_demo;
					LOG(printf_P, PSTR("double click index: %u"), dclick_func);
					dclick_func++;
				}
			}
			break;
