...
- [О проекте](#о-проекте)
- [Возможности часов](#возможности-часов)
- [Управление](#управление)
- [Особенности](#особенности)
- [Схема](#схема)
- [Сборка (компиляция и прошивка)](#сборка)
- [Внешние датчики](#внешние-датчики)
- [Отправка текста та экран](#отправка-текста-та-экран)
- [Телеграм](#телеграм)
- [Режим "Кукушка"](#режим-кукушка)
- [Изменения](#изменения)
...
- Показать температуру/погоду/прогноз погоды/цитату/включить проверку матрицы. Каждое следующее двойное нажатие выбирает следующую строку.
...
Альтернативный вариант - установить через git:  
```
git clone https://github.com/SerhiiLe/clock-ws2812b.git
```  
Можно переименовать папку в что-то более короткое, напрмер "clock", это не повлияет на сборку, прошивку и обновления. Затем, для обновления исходного кода достаточно дать команды внутри этой папки:  
```
git fetch origin
git reset --hard origin/main
```  
Все файлы обновяться. Но при этом сотруться изменения, которые Вы сами могли вносить.
Или вариант со слиянием Ваших изменений и кода на сервере:  
```
git stash
git pull origin main
git stash pop
```  
Надо пробовать, что подойдёт конкретно Вам. В крайнем случае всегда можно сказачть архив и начать всё сначала.
...
**Обязательно переименовть файл include/defines.h.sample в include/defines.h, это нужно для того, чтобы при обновлении исходного кода, не пришлось восстанавливать все параметры под Вашу конфигурацию.**
...
Если использовать модуль ESP32c3, то у него второй полноценный UART, и на 3 ножки больше. Это позволяет добавить аппаратный модуль часов и датчики температуры, влажности, давления. А так-же измерять напряжение на аккумуляторе. Я рекомендую для сборки использовать ESP32c3 Supermini.
...
Через эту опцию можно выводить сообщения к примеру из HomeAssistant.

Отправить тект на экран из telegram можно, но без выбора дополнительных параметров.

## Телеграм

Телеграм используется для двух целей: отсылка сообщений о стработке датчиков и управления внешними датчиками.

Телеграм очень удобен для управления в режиме чата разными устройствами, абсолютно бесплатно на данный момент и без необходимости иметь реальные IP адреса. Как пример такого датчика и взаимодействия можно посмотреть мой [проект для полива цветов](https://github.com/SerhiiLe/Watering-esp32). Можно в каждое устройство добавлять своего небольшого бота и общаться с ним непосредственно, но мне удобнее иметь один чат для управления всеми устройствами. Кроме того, к примеру в прошивках [Tasmota](https://tasmota.github.io/docs/) есть возможность запрашивать url для отсылки уведомлений или каких-то действий. Прописать url часов проще и пусть часы занимаются непосредственно отправкой и повторами, если с первого раза сообщение не ушло.

Подключить телеграм просто, надо зарегистрировать своего бота, вписать его имя и токен в настройках раздела "охрана". После подключения к боту узнать какой номер Вашего чата командой /chatid и вписать его в строку разрешённых чатов. Всё.

Если телеграм не используется, то достаточно оставить пустым поле с токеном путым. Все функции телеграм отключатся.

Чат в этом боте простейший. Все команды можно посмотреть командой /help

Для удалённого управления устройствами выбор не такой большой. Самый простой для непосредственного управления и без установки специализированных програм-клиентов и серверов это телеграм. На втором месте MQTT, но тут уже есть ограниченя в публичных серверах. Однако для "умного дома" обычно выбирается именно MQTT и свой сервер в паре с HomeAssistant. Есть ещё проприетарные сервисы, но там свои нюансы. Различные другие чаты вроде WhatsApp, Viber, Signal - не годятся, так как они имеют очень ограниченые возможности и в лучшем случае туда можно отсылать сообщения, опять же через сторонние сервисы, но не двухстороння связь.

## Режим "Кукушка"

Дополнительный режим, находится в разделе "дополнительно", который проигрывает заданные файлы в начале каждого часа.

Может проиграть только два файла за один раз. Это собственно "кукушка", что угодно, желательно короткое. Всегда один и тот-же файл. Второй файл это время. Файлы времени должны быть пронумерованы последовательно, от полночи до 23 часов. Указывается в настройках только первый файл - полночь, остальные вычисляются как "полночь" + час. Если "полночь" это файл номер 001.mp3, то 23 часа это файл 024.mp3 (1+23). Файл "кукушка" должен находится в одной папке в файлами времени.

[Пример содержимого на флешке](https://drive.google.com/file/d/17CfILP6uh817H38tvFNAVm0zW9wZSD93/view?usp=sharing) предоставлен [Віктор Іванович Вождаєнко](https://github.com/Richy2110). Файлы кукушки лежат в папке 006. 001 - файл "полночь", 024 - 23 часа, 025 - "кукушка", 026 - тест. В настройках указывать папку 6, полночь - 1, кукушка 25. Есть поле "check", где можно проверить правильность настроек и громкость.

Общее правило для всех DFPlayer:
- поддерживаются только microSD и microSDHC карточки, это объём до 32Гб! Ищите или специально заказывайте небольшие карточки. (SDXC и SDUC не подходят)
- поддерживается только FAT16 (для microSD до 2Гб), и FAT32 (от 2 до 32ГБ). NTFS, exFAT, ext4 не поддерживаются. 
- файлы могут лежать в корне и не иметь номера, но тогда работоспособность не гарантируется.
- файлы надо раскладывать по папкам. Названия папок строго регламентированы.
- обычные папки, название только цифровое от "01" до "99". Ничего больше.
- специальная папка с названием "mp3". Тут можно поместить больше файлов.
- имена файлов внутри папок 01-99 это три цифры и затем что угодно, потом расширение mp3. От 001_first.mp3 до 255_last.mp3
- имена файлов внутри папки "mp3" это четыре цифры. 0001_test.mp3. От 0001_first.mp3 до 4095_last.mp3
- папки с названием mp3 на конкретном экземпляре могут не работать, надо пробовать.
- на старых моделях чипа номера папок так-же должны быть из трёх цифр. Но номера по прежнему от 001 до 099.
- адресация по номерам работает не очень устойчиво, надёжнее глобальный номер трека. Глобальный номер зависит от очерёдности записи на sdcard.

**Внимание!** Не забыть выбрать или подобрать тип Вашего чипа в include/defines.h !
...
- добавил "кукушку" и исправил много ошибок в коде.
- переименовал файл include/defines.h в include/defines.h.sample, чтобы при обновлении с git не надо было возвращать конфигурацию.
- добавлены иконки погоды, добавлены разные единицы измерения для температуры, давления, скорости ветра. Изменён формат хранения будильников. Завершен перевод на английский язык. Убран больше не работающий forismatic.com
